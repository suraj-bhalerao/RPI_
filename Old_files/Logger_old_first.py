...
class SerialUtility:
    def __init__(self, root):
        ...
        self.log_console.bind("<Key-space>", self.on_space_key_press)
        self.log_console.bind("<MouseWheel>", self.on_mouse_wheel)  # Bind mouse wheel event

        # Ensure the log_console has focus so it can capture the key events
        self.log_console.focus()
        ...
        while True:
            try:
                ...
                if len(current_ports) > len(previous_ports):
                    self.serial_port = serial.Serial(current_ports[0], baudrate=115200)
    ...
    def start_logging(self):
        ...
        if not self.log_file:
            self.log_file = open(f"serial_log_{host}_{time.strftime('%Y%m%d_%H%M%S')}.log", 'a')
